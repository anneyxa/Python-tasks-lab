...
class Perceptron:
    ...
    def __init__(self, learning_rate=0.01, number_of_iterations=1000):  # czemu muszę to ustalać już na etapie tworzenia?
    ...
    def train(self, train_data, train_label):
        ...
        self.weights = np.zeros(columns)  # inicjalizacja wag, na początku zera # lepiej to zrobić w konstruktorze
        self.bias = 0
        for x in range(self.number_of_iterations):  # wykonujemy np. 1000 iteracji, aby nauczyć model
            for i, current_data in enumerate(train_data):  # iterujemy się po danych wejściowych    # polecam zip
    ...
    def check_precision(self, actual, predicted):
        precision = np.sum(actual == predicted) / len(actual)   # to nie jest precision
        return precision
